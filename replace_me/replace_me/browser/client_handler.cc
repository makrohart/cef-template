...
#include "replace_me/browser/client_handler.h"
...
#include "replace_me/common/resource_util.h"
#include "replace_me/common/binary_value_utils.h"
...
bool ClientHandler::OnProcessMessageReceived(
  ...
  if (ClientHandlerBase::OnProcessMessageReceived(browser, frame,
...
void ClientHandler::OnDownloadUpdated(
  ...
  if (download_item->IsComplete()) {
    // Download complete notification removed (test code)
  }
}
...
bool ClientHandler::OnDragEnter(CefRefPtr<CefBrowser> browser,
  ...
  if ((mask & DRAG_OPERATION_LINK) && !dragData->IsFragment()) {
    // Drag blocking notification removed (test code)
    return true;
  }
...
bool ClientHandler::OnSetFocus(CefRefPtr<CefBrowser> browser,
  ...
  if (ClientHandlerBase::OnSetFocus(browser, source)) {
    return true;
  }
...
bool ClientHandler::OnPreKeyEvent(CefRefPtr<CefBrowser> browser,
  ...
  if (!event.focus_on_editable_field && event.windows_key_code == 0x20) {
    ...
    if (event.type == KEYEVENT_RAWKEYDOWN) {
      // Space bar alert removed (test code)
    }
    return true;
  }
  */
...
void ClientHandler::OnAfterCreated(CefRefPtr<CefBrowser> browser) {
  ...
  ClientHandlerBase::OnAfterCreated(browser);
...
void ClientHandler::OnBeforeClose(CefRefPtr<CefBrowser> browser) {
  ...
  ClientHandlerBase::OnBeforeClose(browser);
...
void ClientHandler::OnLoadingStateChange(CefRefPtr<CefBrowser> browser,
  ...
  ClientHandlerBase::OnLoadingStateChange(browser, isLoading, canGoBack,
                                          canGoForward);
...
void ClientHandler::OnRenderProcessTerminated(CefRefPtr<CefBrowser> browser,
  ...
  CEF_REQUIRE_UI_THREAD();
  ClientHandlerBase::OnRenderProcessTerminated(browser, status, error_code,
                                               error_string);
...
void ClientHandler::ShowSSLInformation(CefRefPtr<CefBrowser> browser) {
  ...
  config->with_osr = is_osr_;
  config->url = "data:text/html," + ss.str();
  MainContext::Get()->GetRootWindowManager()->CreateRootWindow(
      std::move(config));
}
