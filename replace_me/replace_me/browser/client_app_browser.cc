...
#include "replace_me/browser/main_message_loop_external_pump.h"
...
namespace client {
    ...
    // static
    void ClientAppBrowser::RegisterCookieableSchemes(std::vector<std::string>& cookieable_schemes) {
    }

    // static
    void ClientAppBrowser::CreateDelegates(DelegateSet& delegates) {
        delegates.insert(new browser::ClientBrowserDelegate);
    }
    ...
    void ClientAppBrowser::OnBeforeCommandLineProcessing(
        ...
        if (process_type.empty()) {
            // Disable web security to allow file:// protocol to load resources without CORS
            command_line->AppendSwitch("disable-web-security");
            command_line->AppendSwitch("allow-file-access-from-files");
            command_line->AppendSwitch("allow-universal-access-from-file-urls");
