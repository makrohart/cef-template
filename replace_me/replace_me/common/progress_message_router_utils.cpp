...
#include "progress_message_router_utils.h"
...
namespace {
    ...
    constexpr size_t kIsProgress = 4;
    ...
    CefRefPtr<CefProcessMessage> BuildBrowserListMsg(const CefString& name,
        ...
        args->SetBool(kIsPersistent, false);
    ...
    class EmptyResponseBuilder final : public cef_message_router_utils::BrowserResponseBuilder {
    public:
    ...
    class BinaryResponseBuilder final : public cef_message_router_utils::BrowserResponseBuilder {
    public:
    ...
    class StringResponseBuilder final : public cef_message_router_utils::BrowserResponseBuilder {
    public:
    ...
    class SPMResponseBuilder final : public cef_message_router_utils::BrowserResponseBuilder {
    public:
...
ProgressBrowserMessage ParseBrowserMessage(
    ...
    if (auto args = message->GetArgumentList()) {
        ...
        const bool is_progress = args->GetBool(kIsProgress);

        if (is_success) {
            DCHECK_EQ(args->GetSize(), 4U);
            const auto payload_type = args->GetType(kBrowserPayload);
            if (payload_type == CefValueType::VTYPE_STRING) {
                return { context_id, request_id, is_success, is_progress, kNoError, args->GetString(kBrowserPayload) };
            }

            if (payload_type == CefValueType::VTYPE_BINARY) {
                return { context_id, request_id, is_success, is_progress, kNoError, new BinaryValueBuffer(message, args->GetBinary(kBrowserPayload)) };
            }

            DCHECK(payload_type == CefValueType::VTYPE_NULL);
            return { context_id, request_id, is_success, is_progress, kNoError, new EmptyBinaryBuffer() };
        }

        DCHECK_EQ(args->GetSize(), 5U);
        return { context_id, request_id, is_success, is_progress, args->GetInt(3), args->GetString(4) };
    }
    ...
    if (region && region->IsValid()) {
        ...
        if (header->is_binary) {
            return { header->context_id, header->request_id, true, false, kNoError, new SharedMemoryRegionBuffer(region, sizeof(BrowserMsgHeader)) };
        }
        return { header->context_id, header->request_id, true, false, kNoError, GetStringFromMemory<BrowserMsgHeader>(region->Memory(), region->Size()) };
    }
